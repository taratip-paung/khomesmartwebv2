...
const HUB = new THREE.Vector3(0, 3.3, 0) // house roof ridge — ecosystem hub
 ...
 */
const links = [
  { id: 'solar', name: 'CONNECTION_SOLAR', to: [0.9, 2.9, 0], color: '#ffc857', lift: 0.6, pulses: 2 },
  { id: 'rnd', name: 'CONNECTION_RND', to: [RND_ORIGIN[0], 2.7, RND_ORIGIN[2]], color: '#7cf5c2', lift: 2.2, pulses: 3 },
  { id: 'network', name: 'CONNECTION_NETWORK', to: [NETWORK_ORIGIN[0], 5.6, NETWORK_ORIGIN[2]], color: '#35d6ff', lift: 2.6, pulses: 3 },
  { id: 'cloud', name: 'CONNECTION_CLOUD', to: [CLOUD_ORIGIN[0], 3.9, CLOUD_ORIGIN[2]], color: '#a78bfa', lift: 2.4, pulses: 3 },
]
...
function Link({ link, pulseCount }) {
  ...
  useFrame(({ clock }) => {
    ...
    if (lineRef.current) lineRef.current.material.opacity = Math.min(1, 0.35 * k)
  ...
  return (
    <group name={link.name}>
      <Line ref={lineRef} points={points} color={link.color} lineWidth={1} transparent opacity={0.35} depthWrite={false} />
      {Array.from({ length: pulseCount }).map((_, i) => (
        <mesh key={i} ref={(el) => (pulses.current[i] = el)} raycast={() => null}>
          <sphereGeometry args={[0.055, 6, 6]} />
          <meshBasicMaterial color={link.color} transparent opacity={0.9} depthWrite={false} blending={THREE.AdditiveBlending} />
        </mesh>
      ))}
    </group>
  )
